...
def get_aspect_ratio_for_view(spacing_ijk, view_type, transpose=True):
    ...
    if view_type == 'axial':
        # Axial (Z-slice): derived from axes i (0) and j (1)
        # Viewer Logic: np.flip(slice.T, 1) implies Transpose
        # Transpose swaps rows/cols: 
        #   Rows correspond to axis j (s_j)
        #   Cols correspond to axis i (s_i)
        return s_j / s_i if transpose else s_i / s_j

    elif view_type == 'sagittal':
        # Sagittal (X-slice): derived from axes j (1) and k (2)
        # Viewer Logic: np.flip(slice.T, 1)
        # Transpose swaps rows/cols:
        #   Rows correspond to axis k (s_k)
        #   Cols correspond to axis j (s_j)
        return s_k / s_j if transpose else s_j / s_k

    elif view_type == 'coronal':
        # Coronal (Y-slice): derived from axes i (0) and k (2)
        # Viewer Logic: np.flip(slice.T, 1)
        # Transpose swaps rows/cols:
        #   Rows correspond to axis k (s_k)
        #   Cols correspond to axis i (s_i)
