...
class MedicalViewer_v1_8:
    ...
    def _setup_viewer_grid(self, parent):
        ...
        # Dose Hist (Bottom of Hist Frame)
    ...
    def update_plots(self):
        ...
        for i, ax in enumerate(self.axes):
            ...
            # Features
            h, w = slices[i].shape
            if i==0: sp = (sx, sy) # Transposed -> (y, x) but physical spacing matches axis? 
            # Img (X, Y). T -> (Y, X). Axis 0 is Y (sy), Axis 1 is X (sx). 
            # In plot: Y_plot is vertical (orig X axis flipped?), X_plot is horizontal (orig Y axis).
            # Wait, v1.3: flip(T, 1).
            # Ax: (X, Y) -> T -> (Y, X). flip 1 -> (Y, -X). 
            # Vertical axis: Y (spacing sy). Horizontal axis: X (spacing sx).
            # Sag: (X, Y, Z). Slice X. -> (Y, Z). T -> (Z, Y). flip 1 -> (Z, -Y).
            # Vertical: Z (sz). Horizontal: Y (sy).
            # Cor: (X, Y, Z). Slice Y. -> (X, Z). T -> (Z, X). flip 1 -> (Z, -X).
            # Vertical: Z (sz). Horizontal: X (sx).
            
            # Note: _draw_ticks takes (spacing_x_plot, spacing_y_plot)
