...
class MedicalViewerApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Python Medical Dose Viewer (Optimized)")
        ...
        # --- Graphics Storage (The Strategy 1 Optimization) ---
        # We store references to the matplotlib "Artists" so we can update them
        ...
        self.artists = {
            ...
            'contours': [None, None, None] # Contour sets (need special handling)
        }
    ...
    def _init_plots(self):
        ...
        # Initialize Image Artists with Dummy Data (Speed Trick)
        # We create them once here, and only update their data later.
        ...
        for i, ax in enumerate(self.image_axes):
            ...
            # 2. Dose Image Artist (Hidden initially)
    ...
    def load_file(self, file_type):
        ...
        try:
            ...
            if file_type == 'img':
                ...
                # Update Histograms ONLY on load (Expensive operation)
    ...
    def _update_histograms(self):
        # Only called on load or full refresh, not during scroll
        self.ax_hist_img.clear()
        if self.img_data is not None:
            # Downsample for speed in histogram calculation
    ...
    def update_plots(self):
        ...
        def update_view_idx(idx, slice_data, dose_slice, cur_cross_x, cur_cross_y, max_x, max_y):
            ...
            # --- ADD THIS LINE BELOW ---
            # Fixes the "small icon" bug by resizing the image to fit the new data
            ...
            # 4. Update Crosshairs
            # Flipped logic from original code applied here
            ...
            # Labels
            labels = [['A', 'P', 'R', 'L'], ['S', 'I', 'A', 'P'], ['S', 'I', 'R', 'L']]
            top, bot, left, right = labels[idx]
            
            # Re-draw labels only if needed (Skipping for optimization, they don't move)
        ...
        # IMPORTANT: Use draw_idle for non-blocking UI updates
