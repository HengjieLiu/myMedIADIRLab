...
# --- HELPER CLASSES (From utils_display.py / Snippets) ---
...
class SpatialTransformer(nn.Module):
    ...
    def forward(self, src, flow, mode='bilinear', padding_mode='border', align_corners=True):
        ...
        return nnf.grid_sample(src, new_locs, mode=mode, padding_mode=padding_mode, align_corners=align_corners)

def mk_grid_img_3d(grid_step, line_thickness=1, grid_size=(160, 224, 192), axis_to_slice=0):
    grid_img_np = np.zeros(grid_size, dtype=np.float32)
    
    if axis_to_slice == 0:
        for val in range(0, grid_img_np.shape[1], grid_step):
            grid_img_np[:, val:val+line_thickness, :] = 1
    ...
    return torch.from_numpy(grid_img_np[None, None, ...])
...
class JDet:
    def __call__(self, disp):
        ...
        gradx_disp = np.stack([scipy.ndimage.correlate(disp_np[:, i], gradx, mode='constant', cval=0.0) for i in range(3)], axis=1)
        grady_disp = np.stack([scipy.ndimage.correlate(disp_np[:, i], grady, mode='constant', cval=0.0) for i in range(3)], axis=1)
        gradz_disp = np.stack([scipy.ndimage.correlate(disp_np[:, i], gradz, mode='constant', cval=0.0) for i in range(3)], axis=1)
        ...
        jacdet = np.pad(jacdet, ((0,0), (2,2), (2,2), (2,2)), mode='constant', constant_values=1)
        return jacdet.squeeze()
...
class MedicalViewerApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Python Medical Viewer v1.6 (DVF Support)")
        ...
        self.grid_warped = [None, None, None] # List of 3 numpy arrays (H,W,D) for each view axis
    ...
    def load_file(self, ftype):
        ...
        try:
            ...
            if ftype == 'img':
                ...
                # 1. Compute JDet
                disp_batch = self.dvf_data[np.newaxis, ...] # Add batch dim
                self.jdet_data = JDet()(disp_batch)
                print(f"JDet computed. Range: {self.jdet_data.min():.2f} to {self.jdet_data.max():.2f}")
                
                # 2. Compute Warped Grids (3 Volumes)
                # Create ST
                st = SpatialTransformer(self.dims).cpu() # Use CPU
                disp_tensor = torch.from_numpy(self.dvf_data).float().unsqueeze(0) # (1, 3, H, W, D)
                
                # Generate and warp grid for each axis view optimization
                for axis in range(3):
                    grid_img = mk_grid_img_3d(grid_step=10, grid_size=self.dims, axis_to_slice=axis)
                    warped = st(grid_img, disp_tensor)
                    self.grid_warped[axis] = warped.detach().squeeze().numpy()
    ...
    def update_plots(self):
        ...
        def update_ax(idx, slice_img, slice_dose, slice_jdet, slice_grid, cx, cy, max_x, max_y):
            ...
            if self.jdet_data is not None and self.show_jdet.get():
                ...
                if self.log_jdet.get():
                    # Shift center 1 -> 0. Handle negatives.
                    # Safe log: log(max(val, eps))
                    # But if we want 0 centered, log(1)=0.
                    jd_disp = np.log(np.maximum(slice_jdet, 1e-6))
                    # Adjust clim for log? Log(3) ~ 1.1, Log(0.3) ~ -1.2.
                    # Let's use user range but interpreted as log? No, confusing.
                    # Standard behavior: Center 0, range -1 to 1?
                    jd.set_data(jd_disp)
                    jd.set_clim(-1, 1) # Standard log range
                else:
                    jd.set_clim(self.jdet_min, self.jdet_max)
                
                # Contours for JDet < 0 (Folding)
                ax = self.axes[idx]
        ...
        # Extract Slices (Note: Standardize on Transpose+Flip for display)
        # Axial (Z)
    ...
    def _on_mouse_press(self, event):
        ...
        if event.button == 1:
            ...
            # Map click to slice idx (Inverse of display logic)
    ...
    def _on_mouse_move(self, event):
        ...
        if self.active_zoom_ax:
            ...
            self.update_plots()
        elif self.active_pan_ax == event.inaxes:
             # Basic pan logic (simplified)
             pass 
