...
class myDicomImageReader:
    def __init__(self, dicom_path, modality='CT'):
        """
        ...
        ValueError
            If folder is empty, or if folder contains multiple DICOM series
        """
        ...
        if os.path.isfile(dicom_path):
            ...
            self.series_file_names = sitk.ImageSeriesReader.GetGDCMSeriesFileNames(
                dicom_path, series_IDs[0]
            )
            
            if len(self.series_file_names) == 0:
                raise ValueError(f"No DICOM files found in series: {dicom_path}")
            elif len(self.series_file_names) == 1:
                # Single file in series
                self.is_series = False
                self.file_path = self.series_file_names[0]
                print(f"[Info] Reading single DICOM file from folder: {os.path.basename(self.file_path)}")
            else:
                # Multiple files - it's a series
                self.is_series = True
                self.file_path = self.series_file_names[0]  # First slice for metadata
                print(f"[Info] Reading DICOM series from folder: {dicom_path}")
                print(f"[Info] Series contains {len(self.series_file_names)} slices")
        else:
            raise ValueError(f"Path is neither a file nor a directory: {dicom_path}")
        
        # Validate that it's an DICOM file with the correct modality
        try:
    ...
    def _get_file_names_from_path(self, dicom_path):
        """
        Extract file names from a DICOM path (file or directory).
        
        Helper method to get a list of DICOM file paths from either a single file
        or a directory containing a DICOM series. This is used primarily for the
        geometry override feature.
        ...
        if os.path.isfile(dicom_path):
            ...
            if len(series_IDs) == 0:
                ...
                raise ValueError(f"Multiple DICOM series found ({len(series_IDs)} series) in directory: {dicom_path}. "
                               f"Please provide a directory with exactly one series.")
